...
def save_memory(owner_id, content, category="general", importance=0.65, confidence=1.0, source="chat"):
    ...
    with db() as c:
        ...
        importance=max(importance,excluded.importance),confidence=max(confidence,excluded.confidence),updated_at=CURRENT_TIMESTAMP""",
        (str(owner_id),content,category,importance,confidence,source))
        row=c.execute("SELECT id FROM memories WHERE owner_id=? AND content=?",(str(owner_id),content)).fetchone()
        memory_id=row["id"]
        c.execute("INSERT OR REPLACE INTO memory_embeddings(memory_id,vector) VALUES(?,?)",(memory_id,pack(embed(content))))
    return memory_id
...
def forget_target(text):
    m=re.match(r"\s*(?:please\s+)?forget(?:\s+that)?\s+(.+)",text,re.I|re.S)
    return m.group(1).strip() if m else None
...
def forget_memories(owner_id, query):
    ...
    with db() as c:
        c.executemany("DELETE FROM memories WHERE id=? AND owner_id=?",[(i,str(owner_id)) for i in ids])
    return len(ids)

def auto_memory_candidates(text):
    patterns=[r"\bmy (?:name|company|business|job|role|birthday|favorite|favourite|preference|office|address)\b.+",r"\bi (?:am|work at|work for|own|prefer|like|use|live in|live at)\b.+"]
...
def search_memories(owner_id,query,limit=MEMORY_RESULTS):
    ...
    with db() as c:
        rows=c.execute("""SELECT m.*,e.vector FROM memories m LEFT JOIN memory_embeddings e ON e.memory_id=m.id
        WHERE m.owner_id=? ORDER BY m.importance DESC,m.updated_at DESC LIMIT 500""",(str(owner_id),)).fetchall()
    scored=[]
    for r in rows:
        low=r["content"].lower(); hits=sum(1 for t in qterms if t in low); phrase=1 if query.lower().strip() in low else 0
        vec=unpack(r["vector"]); semantic=cosine(qvec,vec) if vec else cosine(qvec,embed(r["content"]))
        score=hits*2.5+phrase*4+semantic*6+float(r["importance"])*1.5+min(int(r["access_count"]),20)*0.02
...
def search_documents(owner_id,query,limit=DOCUMENT_RESULTS):
    qterms=terms(query); qvec=embed(query)
    with db() as c:
        rows=c.execute("""SELECT dc.id,dc.content,d.original_name,d.id document_id,e.vector
        FROM document_chunks dc JOIN documents d ON d.id=dc.document_id LEFT JOIN chunk_embeddings e ON e.chunk_id=dc.id
        WHERE d.owner_id=? ORDER BY dc.id DESC LIMIT 2500""",(str(owner_id),)).fetchall()
    scored=[]
    for r in rows:
        low=r["content"].lower(); keyword=sum(min(low.count(t),5) for t in qterms)
        vec=unpack(r["vector"]); semantic=cosine(qvec,vec) if vec else cosine(qvec,embed(r["content"]))
        score=keyword*1.8+semantic*7
