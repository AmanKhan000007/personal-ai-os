...
from .memory import save_memory, explicit_memory, auto_memory_candidates
from .llm import ask
...
async def tg_api(method, payload=None):
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        r=await client.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/{method}",json=payload or {})
...
async def tg_send(chat_id,text):
    for start in range(0,len(text),3900):
        await tg_api("sendMessage",{"chat_id":chat_id,"text":text[start:start+3900]})
...
async def index_file(owner_id, original, data, mime="application/octet-stream"):
    ...
    with db() as c:
        cur=c.execute("INSERT INTO documents(owner_id,original_name,stored_name,path,mime_type,size_bytes,extracted_text) VALUES(?,?,?,?,?,?,?)",(str(owner_id),original,stored,str(path),mime,len(data),text))
        doc_id=cur.lastrowid
        c.executemany("INSERT INTO document_chunks(document_id,chunk_index,content) VALUES(?,?,?)",[(doc_id,i,ch) for i,ch in enumerate(chunks(text))])
...
async def process_text(channel,owner_id,text):
    ...
    if explicit:
        save_memory(owner_id,explicit,importance=0.95,source="explicit")
    else:
        for candidate in auto_memory_candidates(text): save_memory(owner_id,candidate,importance=0.7,confidence=0.85,source="automatic")
    answer,provider=await ask(owner_id,text)
    save_chat(channel,owner_id,"assistant",answer); log(channel,owner_id,"chat",provider)
    return answer
...
def home():
    return f"""<html><head><meta name=viewport content='width=device-width'><style>{CSS}</style></head><body><div class=wrap><h1>{html.escape(APP_NAME)}</h1><p class=muted>Private AI with persistent memory and document search.</p><div class=card><h2>Chat</h2><form method=post action=/playground><textarea name=message required placeholder='Ask anything or say Remember that...'></textarea><input type=password name=admin_token required placeholder='Admin token'><button>Ask</button></form></div><div class=card><h2>Upload document</h2><form method=post action=/upload enctype=multipart/form-data><input type=file name=file required><input type=password name=admin_token required placeholder='Admin token'><button>Upload</button></form></div><div class=card><a href=/health>Health check</a></div></div></body></html>"""
...
async def telegram_webhook(req:Request):
    update=await req.json(); msg=update.get("message") or update.get("edited_message") or {}
    sender=str((msg.get("from") or {}).get("id","")); chat_id=(msg.get("chat") or {}).get("id")
    ...
    try:
        if msg.get("document"):
            d=msg["document"]; original=Path(d.get("file_name") or "document").name
            ext=Path(original).suffix.lower()
            if ext not in ALLOWED:
                await tg_send(chat_id,f"Unsupported document type {ext}. Allowed: {', '.join(sorted(ALLOWED))}"); return {"ok":True}
            info=await tg_api("getFile",{"file_id":d["file_id"]}); file_path=info["file_path"]
            async with httpx.AsyncClient(timeout=90) as client:
                r=await client.get(f"https://api.telegram.org/file/bot{TELEGRAM_BOT_TOKEN}/{file_path}"); r.raise_for_status(); data=r.content
            doc_id,chars=await index_file(sender,original,data,d.get("mime_type", ""))
            log("telegram",sender,"upload",original)
            await tg_send(chat_id,f"✅ Indexed {original}\nDocument ID: {doc_id}\nCharacters indexed: {chars}\n\nYou can now ask me questions about it.")
            caption=(msg.get("caption") or "").strip()
            if caption:
                answer=await process_text("telegram",sender,caption); await tg_send(chat_id,answer)
            return {"ok":True}
...
@app.get("/health")
def health(): return {"ok":True,"app":APP_NAME,"telegram_configured":bool(TELEGRAM_BOT_TOKEN and TELEGRAM_OWNER_ID),"provider":os.getenv("LLM_PROVIDER","gemini")}
