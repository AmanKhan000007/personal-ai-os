...
CREATE TABLE IF NOT EXISTS media (id INTEGER PRIMARY KEY AUTOINCREMENT,owner_id TEXT NOT NULL,media_type TEXT NOT NULL,original_name TEXT NOT NULL,stored_name TEXT NOT NULL,path TEXT NOT NULL,mime_type TEXT,size_bytes INTEGER NOT NULL,description TEXT NOT NULL DEFAULT '',label TEXT NOT NULL DEFAULT '',created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE INDEX IF NOT EXISTS idx_media_owner_label ON media(owner_id,label,created_at);
...
def init_db():
 with db() as conn:
  ...
  if 'notified_at' not in task_cols:conn.execute("ALTER TABLE tasks ADD COLUMN notified_at TEXT")
