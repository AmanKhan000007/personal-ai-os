...
def parse_due(phrase):
 now=local_now();p=phrase.lower().strip()
 ...
 if m:
  ...
  d=now+(timedelta(days=1) if day=='tomorrow' else timedelta());return d.replace(hour=h,minute=mi,second=0,microsecond=0)
...
def natural_task(owner_id,text):
 ...
 body=s[len(prefix):].strip();due=parse_due(body);clean=re.sub(r"\s+in\s+\d+\s*(?:minutes?|min|hours?|days?)\s*$","",body,flags=re.I);clean=re.sub(r"\s+(?:today|tomorrow)(?:\s+at)?\s+\d{1,2}(?::\d{2})?\s*(?:am|pm)?\s*$","",clean,flags=re.I);clean=re.sub(r"\s+tomorrow\s*$","",clean,flags=re.I).strip();tid=add_task(owner_id,clean,due.isoformat(timespec='minutes') if due else None,"natural_language")
 return f"✅ Task #{tid} added: {clean}"+(f"\n⏰ Due: {due.strftime('%d %b %Y, %I:%M %p')} ({TIMEZONE})" if due else "")
