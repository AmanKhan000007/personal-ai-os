...
async def gemini(prompt):
 ...
 client=genai.Client(api_key=key);model=os.getenv("GEMINI_MODEL","gemini-2.5-flash")
 response=await client.aio.models.generate_content(model=model,contents=prompt);return (response.text or "").strip()
async def cloudflare(prompt):
 ...
 async with httpx.AsyncClient(timeout=60) as client:
  r=await client.post(f"https://api.cloudflare.com/client/v4/accounts/{account}/ai/run/{model}",headers={"Authorization":f"Bearer {token}"},json={"messages":[{"role":"system","content":SYSTEM},{"role":"user","content":prompt}]});r.raise_for_status();data=r.json()
...
async def groq(prompt):
 ...
 async with httpx.AsyncClient(timeout=60) as client:
  r=await client.post("https://api.groq.com/openai/v1/chat/completions",headers={"Authorization":f"Bearer {key}","Content-Type":"application/json"},json={"model":model,"messages":[{"role":"system","content":SYSTEM},{"role":"user","content":prompt}]});r.raise_for_status();data=r.json()
 return data['choices'][0]['message']['content'].strip()
async def ask(owner_id,message):
 prompt=build_prompt(owner_id,message);primary=os.getenv("LLM_PROVIDER","gemini").lower().strip();configured=[p.strip().lower() for p in os.getenv("LLM_FALLBACKS","groq,cloudflare,gemini").split(',') if p.strip()];providers=[]
